...
img = cv2.imread('output3/telefe.png', 3)# matriz, cada el es un RGB
    ...
    for iteracion in range(iteraciones):
        ...
        for channel in range(3):
            ...
            try:
                ...
                fix_value = np.where(canal < 5, np.zeros(canal.shape), np.ones(canal.shape))
                fix_value = np.where(fix_value > 251, np.zeros(canal.shape), np.ones(canal.shape))
        ...
        im.save("output3/folder/image%d.png" % iteracion)
