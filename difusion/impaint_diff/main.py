...
def procesar(imagen, mask, iteraciones):
    ...
    for iteracion in range(iteraciones):
        ...
        #imagen=cv2.ximgproc.anisotropicDiffusion(imagen, alpha= 0.01,K = 10, niters = 2)
        ...
        for contorno in range(len(cnts)):
            ...
            for index,border_point in enumerate(cnts[contorno]):
                ...
                comp1 = Ln(i+1,j,imagen_LUV) - Ln(i-1,j,imagen_LUV)
                comp2 = Ln(i,j+1,imagen_LUV) - Ln(i,j-1,imagen_LUV)
                ...
                imagen_LUV[i,j] = imagen_LUV[i,j] + delta_t * It_n

                #imagen[y, x] = [0, 0, 0]
                #if(cnt_diff == number_of_diffusions):
...
#                cnt_diff = 0
        ...
        if iteracion % 1 == 0:
            ...
            #im = Image.fromarray(cv2.cvtColor(imagen_LUV, cv2.COLOR_Luv2RGB))
            #im = Image.fromarray(cv2.cvtColor(imagen, cv2.COLOR_BGR2RGB))
            im.save("output3/imagen" + str(iteracion) + ".jpeg")
            #plt.plot(gx[350,:]) # quiero ver la derivada en la columna 500
            # si accedo a gx[400,:] obtengo información horizontal
            # si accedo a gx[:,500] obtengo info vertical
            #plt.show()
